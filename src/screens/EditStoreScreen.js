...
export default function EditStoreScreen({ route, navigation }) {
    ...
    const handleSave = async () => {
        ...
        const storeData = { name, qrCodeValue: qrValue, location: new GeoPoint(lat, lon) };
        try {
            if (isEditing) {
                ...
                if (user) {
                    ...
                    await updateDoc(adminUserRef, { storeIds: arrayUnion(newStoreRef.id) });
                }
            }
            navigation.goBack();
        } catch (error) {
            console.error("Mağaza kaydedilirken hata:", error); Alert.alert("Hata", "Mağaza kaydedilirken bir sorun oluştu.");
    ...
    return (
        <ScrollView style={[styles_edit.container, { backgroundColor: themeData.background }]}>
            ...
            <TextInput style={[styles_edit.input, { backgroundColor: themeData.card, color: themeData.text }]} value={name} onChangeText={setName} placeholder="Örn: Ataşehir Mağazası" />
            <Text style={[styles_edit.label, { color: themeData.text }]}>QR Kod Değeri</Text>
            <TextInput style={[styles_edit.input, { backgroundColor: themeData.card, color: themeData.text }]} value={qrValue} onChangeText={setQrValue} placeholder="Örn: ATASEHIR_MAVI_2025" />
            <Text style={[styles_edit.label, { color: themeData.text }]}>Enlem (Latitude)</Text>
            <TextInput style={[styles_edit.input, { backgroundColor: themeData.card, color: themeData.text }]} value={latitude} onChangeText={setLatitude} keyboardType="numeric" placeholder="Örn: 40.9895" />
            <Text style={[styles_edit.label, { color: themeData.text }]}>Boylam (Longitude)</Text>
            <TextInput style={[styles_edit.input, { backgroundColor: themeData.card, color: themeData.text }]} value={longitude} onChangeText={setLongitude} keyboardType="numeric" placeholder="Örn: 29.1126" />
            <TouchableOpacity style={styles_edit.saveButton} onPress={handleSave} disabled={loading}><Text style={styles_edit.saveButtonText}>{loading ? "Kaydediliyor..." : "Kaydet"}</Text></TouchableOpacity>
        </ScrollView>
    );
}
